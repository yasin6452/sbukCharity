...
import { getMedicalCenter, createMedicalCenter, updateMedicalCenter } from '@/services/data'
...
const validationSchema = z.object({
    ...
    phoneNumber: z.string().regex(/^0\d{2,3}-?\d{7,8}$/, { message: 'شماره تلفن ثابت معتبر نیست (مثال: 021-12345678)' }),
    ...
    services: z.string().min(10, { message: 'لیست خدمات الزامی است و حداقل ۱۰ کاراکتر باشد' }),
    workingHours: z.string().optional(),
    contactPersonName: z.string().min(3, { message: 'نام مسئول حداقل باید ۳ کاراکتر باشد' }),
    contactPersonPhone: z.string().regex(/^09\d{9}$/, { message: 'شماره موبایل مسئول معتبر نیست (مثال: 09123456789)' }),
...
const centerTypeOptions = [
    { value: 'بیمارستان', label: 'بیمارستان' },
    { value: 'کلینیک تخصصی', label: 'کلینیک تخصصی' },
    { value: 'درمانگاه عمومی', label: 'درمانگاه عمومی' },
    { value: 'آزمایشگاه تشخیص طبی', label: 'آزمایشگاه تشخیص طبی' },
    { value: 'مرکز تصویربرداری', label: 'مرکز تصویربرداری' },
    { value: 'مرکز فیزیوتراپی', label: 'مرکز فیزیوتراپی' },
...
const MedicalCenterForm = () => {
    ...
    const { control, handleSubmit, reset, watch, formState: { errors } } = useForm<FormSchema>({
        ...
        defaultValues: {
            ...
            website: '', services: '', workingHours: '',
            contactPersonName: '', contactPersonPhone: '',
            licenseNumber: '', description: '', licenseFile: null
        }
    });
            ...
            const fetchCenterData = async () => {
                ...
                try {
                    ...
                    if (response.ok && response.data) {
                        ...
                        toast.push(<Notification title="خطا" type="danger">{response.message || 'مرکز درمانی یافت نشد.'}</Notification>);
    ...
    const onSubmit = async (data: FormSchema) => {
        ...
        try {
            const response = isEditMode && id
                ? await updateMedicalCenter(parseInt(id), formData)
                : await createMedicalCenter(formData);
            ...
            <Card className="max-w-[1000px] mx-auto">
                <div className="p-6">
                    <h4 className="text-xl font-semibold mb-6">
                        {isEditMode ? 'ویرایش اطلاعات مرکز درمانی' : 'ثبت مرکز درمانی جدید'}
                    </h4>
                    <Form onSubmit={handleSubmit(onSubmit)}>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-3">
                            ...
                            <FormItem label="نام مرکز" invalid={!!errors.name} errorMessage={errors.name?.message}>
                                <Controller name="name" control={control} render={({ field }) => <Input {...field} placeholder="مثال: بیمارستان شفا" />} />
                            </FormItem>
                            <FormItem label="نوع مرکز" invalid={!!errors.type} errorMessage={errors.type?.message}>
                                <Controller name="type" control={control} render={({ field }) => (
                                    <Select options={centerTypeOptions} value={centerTypeOptions.find(opt => opt.value === field.value)} onChange={opt => field.onChange(opt?.value)} placeholder="انتخاب نوع مرکز" />
                                )}/>
                            </FormItem>
                            <FormItem label="ایمیل" invalid={!!errors.email} errorMessage={errors.email?.message}>
                                <Controller name="email" control={control} render={({ field }) => <Input {...field} type="email" placeholder="center@example.com" />} />
                            </FormItem>
                            <FormItem label="شماره تلفن ثابت" invalid={!!errors.phoneNumber} errorMessage={errors.phoneNumber?.message}>
                                <Controller name="phoneNumber" control={control} render={({ field }) => <Input {...field} placeholder="مثال: 02188776655" />} />
                            </FormItem>
                            <FormItem label="وبسایت (اختیاری)" invalid={!!errors.website} errorMessage={errors.website?.message}>
                                <Controller name="website" control={control} render={({ field }) => <Input {...field} placeholder="https://www.center.com" />} />
                            </FormItem>
                            <FormItem label="ساعات کاری (اختیاری)" className="md:col-span-2" invalid={!!errors.workingHours} errorMessage={errors.workingHours?.message}>
                                <Controller name="workingHours" control={control} render={({ field }) => <Input {...field} placeholder="مثال: شنبه تا چهارشنبه ۸ الی ۱۷، پنجشنبه ۸ الی ۱۲" />} />
                            </FormItem>

                            <h5 className="md:col-span-2 text-lg font-medium my-3 p-2 bg-gray-50 rounded">آدرس مرکز</h5>
                            <FormItem label="استان" invalid={!!errors.state} errorMessage={errors.state?.message}>
                                <Controller name="state" control={control} render={({ field }) => <Input {...field} placeholder="مثال: تهران" />} />
                            </FormItem>
                            <FormItem label="شهر" invalid={!!errors.city} errorMessage={errors.city?.message}>
                                <Controller name="city" control={control} render={({ field }) => <Input {...field} placeholder="مثال: تهران" />} />
                            </FormItem>
                            <FormItem label="شهرستان/منطقه" invalid={!!errors.county} errorMessage={errors.county?.message}>
                                <Controller name="county" control={control} render={({ field }) => <Input {...field} placeholder="مثال: منطقه ۶" />} />
                            </FormItem>
                            <FormItem label="جزئیات آدرس" className="md:col-span-2" invalid={!!errors.addressDetail} errorMessage={errors.addressDetail?.message}>
                                <Controller name="addressDetail" control={control} render={({ field }) => <Input {...field} as="textarea" rows={2} placeholder="خیابان، کوچه، پلاک، طبقه..." />} />
                            </FormItem>

                            <h5 className="md:col-span-2 text-lg font-medium my-3 p-2 bg-gray-50 rounded">خدمات و مجوزها</h5>
                            <FormItem label="خدمات ارائه شده" className="md:col-span-2" invalid={!!errors.services} errorMessage={errors.services?.message}>
                                <Controller name="services" control={control} render={({ field }) => <Input {...field} as="textarea" rows={3} placeholder="لیست خدمات را با کاما جدا کنید یا در خطوط جدید بنویسید." />} />
                            </FormItem>
                                ...
                                <Controller name="licenseFile" control={control} render={({ field: { onChange } }) => (
                                    <Upload draggable className="border-2 border-dashed p-4 rounded-lg" onChange={(files) => onChange(files)} showList={false} accept=".pdf,.jpg,.jpeg,.png" uploadLimit={1}>
                                        <div className="text-center">
                                            ...
                                            <p className="font-semibold"><span className="text-gray-800">فایل را بکشید یا </span><span className="text-blue-500">کلیک کنید</span></p>
                                            <p className="mt-1 text-xs text-gray-500">PDF, JPG, PNG (حداکثر 5MB)</p>
                                        </div>
                                    </Upload>
                                )} />
                            ...
                            <FormItem label="شماره موبایل مسئول" invalid={!!errors.contactPersonPhone} errorMessage={errors.contactPersonPhone?.message}>
                                <Controller name="contactPersonPhone" control={control} render={({ field }) => <Input {...field} placeholder="مثال: 09123456789" />} />
                            </FormItem>
                        </div>
                        ...
                        <div className="flex justify-end gap-3 mt-8">
                            ...
                            <Button type="submit" variant="solid" loading={submitting} disabled={submitting}>
                                {isEditMode ? 'بروزرسانی اطلاعات' : 'ثبت مرکز درمانی'}
                            </Button>
                        </div>
                    </Form>
                </div>
