...
import { getMedicalCenters, deleteMedicalCenter, MedicalCenter } from '@/services/data'
...
const MedicalCenterList = () => {
    ...
    const fetchCenters = useCallback(async () => {
        setLoading(true);
        try {
            const response = await getMedicalCenters(currentPage, pageSize, debouncedSearchQuery);
            if (response.ok) {
                ...
                toast.push(
                    <Notification title="خطا" type="danger">
                        {response.message || 'خطا در دریافت لیست مراکز درمانی'}
                    </Notification>
                );
            }
        } catch (error) {
    ...
    const handleAddCenter = () => {
        navigate('/medical-centers/create');
    };
    ...
    const handleView = (center: MedicalCenter) => {
        navigate(`/medical-centers/view/${center.id}`);
    };

    const handleDeleteClick = (center: MedicalCenter) => {
    ...
    const handleDeleteConfirm = async () => {
        ...
        try {
            const response = await deleteMedicalCenter(centerToDelete.id);
            if (response.ok) {
                toast.push(
                    <Notification title="موفقیت" type="success">
                        مرکز درمانی با موفقیت حذف شد.
                    </Notification>
                );
                ...
                toast.push(
                    <Notification title="خطا" type="danger">
                        {response.message || 'خطا در حذف مرکز درمانی.'}
                    </Notification>
                );
            }
        } catch (error) {
        ...
        <div className="min-h-screen w-full p-4">
            <Card className="max-w-[1200px] mx-auto">
                <div className="flex justify-between items-center mb-4 p-4">
                    <h4 className="text-lg font-semibold">مدیریت مراکز درمانی</h4>
                ...
                <div className="px-4 mb-4">
                    <Input
                        ...
                        placeholder="جستجو بر اساس نام، شهر، نوع مرکز..."
                        className="w-full"
                    />
                </div>
                ...
                <div className="p-4">
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center h-60 text-gray-500">
                            <p className="mb-2">{debouncedSearchQuery ? 'هیچ مرکز درمانی با این مشخصات یافت نشد.' : 'هیچ مرکز درمانی ثبت نشده است.'}</p>
                            {debouncedSearchQuery ? (
                                <Button variant="plain" onClick={() => setSearchQuery('')}> پاک کردن جستجو </Button>
                            ) : (
                                <Button variant="solid" onClick={handleAddCenter}> افزودن مرکز درمانی </Button>
                            )}
                        </div>
                    ) : (
                        <>
                            <div className="mb-4 text-sm text-gray-500">
                                نمایش {medicalCenters.length} مرکز از مجموع {totalItems} مرکز
                            </div>
                                    ...
                                    <tbody>
                                        {medicalCenters.map((center) => (
                                            <tr key={center.id} className="hover:bg-gray-50">
                                                ...
                                                <td className="p-3 border-b">{center.type}</td>
            ...
            <Dialog isOpen={deleteConfirmOpen} onClose={() => setDeleteConfirmOpen(false)} onRequestClose={() => setDeleteConfirmOpen(false)}>
                <h5 className="mb-4">تایید حذف مرکز درمانی</h5>
                <p>
                    آیا از حذف مرکز درمانی <span className="font-semibold">"{centerToDelete?.name}"</span> اطمینان دارید؟
                </p>
